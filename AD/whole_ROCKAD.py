"""
wROCKAD: ROCKET-based Whole-Series Anomaly Detector (adapted)

Upstream source (aeon)
----------------------
- Repository: https://github.com/aeon-toolkit/aeon
- File: aeon/anomaly_detection/series/distance_based/_rockad.py
- Commit: fe2a834b162cc0849a702bdffc95b9f4b8c62e7a
- Line ref: L19 (https://github.com/aeon-toolkit/aeon/blob/fe2a834b162cc0849a702bdffc95b9f4b8c62e7a/aeon/anomaly_detection/series/distance_based/_rockad.py#L19)

Original method reference
-------------------------
Theissler, A., Wengert, M., & Gerschner, F. (2023).
"ROCKAD: Transferring ROCKET to Whole Time Series Anomaly Detection."
In: Crémilleux, B., Hess, S., Nijssen, S. (eds) *Advances in Intelligent Data Analysis XXI*.
LNCS 13876, Springer, Cham. https://doi.org/10.1007/978-3-031-30047-9_33

Notes on this adaptation
------------------------
- Adapted for whole-series, collection-level scoring as `wROCKAD`.
- Minor robustness tweaks to reduce numerical/overflow warnings, e.g.:
  * cast ROCKET features to float64,
  * drop non-finite / near-constant feature columns,
  * guard PowerTransformer (Yeo–Johnson) with try/except and fall back if it fails.



- If you redistribute, keep this attribution block and the citation to the ROCKAD paper.
"""
...
# Adapted from aeon (commit fe2a834b162cc0849a702bdffc95b9f4b8c62e7a),
# file: aeon/anomaly_detection/series/distance_based/_rockad.py
# Method: ROCKAD (Theissler et al., 2023)

class wROCKAD(BaseCollectionAnomalyDetector):
    """
    ...
    def _fit(self, X: np.ndarray, y: Optional[np.ndarray] = None) -> "wROCKAD":
        _X = X
        self._inner_fit(_X)
    ...
    def _inner_fit(self, X: np.ndarray) -> None:
        ...
        Xt = self.rocket_transformer_.fit_transform(X).astype(np.float64)
        self.finite_mask_ = np.isfinite(Xt).all(axis=0) & (Xt.var(axis=0) > 1e-4)
        Xt = Xt[:, self.finite_mask_]
    ...
    def _inner_predict(self, X: np.ndarray) -> np.ndarray:
        ...
        # XT: (n_cases, n_kernels*2)
        Xt = self.rocket_transformer_.transform(X).astype(np.float64)
        Xt = Xt[:, self.finite_mask_]
